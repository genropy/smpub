...
def generate_openapi_schema(
    ...
    title: str = "API",
    version: str = "0.1.0",
    description: str = "Auto-generated API"
) -> Dict[str, Any]:
    """
    ...
    for path, handler_info in publisher._openapi_handlers.items():
        ...
        for method_name, method_info in api_schema['methods'].items():
            ...
            operation = {
                "summary": method_info.get('docstring', '').split('\n')[0] if method_info.get('docstring') else method_name,
            ...
            if request_body_properties:
                required_params = [
                    p['name'] for p in method_info['parameters']
                    if p['required']
                ]
